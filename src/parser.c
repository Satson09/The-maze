...
/**
 * concat - concatenates two strings with a path separator
 * @s1: destination string
 * @s2: source string to concatenate
 * Return: New string with @s1 and @s2 separated by a '/'
 */
char *concat(const char *s1, const char *s2)
{
    ...
    result = malloc(len1 + len2 + 2); // +2 for '/' and null terminator
    if (!result)
        return (NULL);

    strcpy(result, s1);
    result[len1] = '/'; // Add path separator
    strcpy(result + len1 + 1, s2); // Copy s2 after the separator
    return (result);
}
